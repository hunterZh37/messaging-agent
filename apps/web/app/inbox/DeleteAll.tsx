...
import { disposableThreadsAction, type DisposableScope } from "./actions";

/**
 * "Delete all · N" on the Safe-to-delete view (spec 10a, 2026-09-11), where
 * "Mark all opened" sits on Unopened. It acts on what the operator is looking
 * at, not on the whole inbox: the window, the project and the money side all
 * narrow it, and N is the threads in that view.
 *
 ...
 */
export function DeleteAll({ scope, count: served }: { scope: DisposableScope; count: number }) {
  const { trash } = useSendGate();
  // Less the threads the list beneath has already let go of (2026-09-15).
  useListAdjustVersion();
  const count = lessGone(served, listAdjust.get(`${scope.folder ?? "inbox"}:disposable`)?.threads);
  const [pending, startTransition] = useTransition();

  function deleteAll() {
    startTransition(async () => {
      const threadIds = await disposableThreadsAction(scope);
