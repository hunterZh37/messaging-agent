...
import { completeGoogleAuth } from "@messaging-agent/core";
...
export async function GET(req: NextRequest) {
  ...
  if (error) return NextResponse.redirect(new URL(`/inboxes?error=${encodeURIComponent(error)}`, origin), 302);
  ...
  try {
    const account = await completeGoogleAuth(cfg, db, { code, codeVerifier: saved.codeVerifier, redirectUri: `${origin}/api/oauth/google/callback` });
