...
describe("folderCounts", () => {
  it("counts pending drafts, mail needing a reply, and sent mail waiting on an answer", () => {
    ...
    expect(folderCounts(db, { since: 0 })).toEqual({ drafts: 1, needsReply: 1, unopened: expect.any(Number), disposable: 0, waiting: 1, hidden: 0, texts: { needsReply: 0, unopened: 0, disposable: 0, hidden: 0 } });
    expect(folderCounts(db, { accountId: "a1", since: 0 })).toEqual({ drafts: 1, needsReply: 1, unopened: expect.any(Number), disposable: 0, waiting: 1, hidden: 0, texts: { needsReply: 0, unopened: 0, disposable: 0, hidden: 0 } });
  });

  it("counts nothing outside the window, and nothing for another inbox", () => {
    const db = testDb();
    seed(db);
    expect(folderCounts(db, { since: 1_000 })).toEqual({ drafts: 0, needsReply: 0, unopened: expect.any(Number), disposable: 0, waiting: 0, hidden: 0, texts: { needsReply: 0, unopened: 0, disposable: 0, hidden: 0 } });
    expect(folderCounts(db, { accountId: "a2", since: 0 })).toEqual({ drafts: 0, needsReply: 0, unopened: expect.any(Number), disposable: 0, waiting: 0, hidden: 0, texts: { needsReply: 0, unopened: 0, disposable: 0, hidden: 0 } });
  });
});
...
describe("the tree's counts agree with the lists they open", () => {
  ...
  it("narrows to one inbox the same way the lists do", () => {
    ...
    expect(folderCounts(db, { accountId: "a2" })).toEqual({ drafts: 0, needsReply: 0, unopened: 0, disposable: 0, waiting: 0, hidden: 0, texts: { needsReply: 0, unopened: 0, disposable: 0, hidden: 0 } });
  });
});
